...
class _AddEditDoctorScreenState extends State<AddEditDoctorScreen> {
  ...
  TimeOfDay _parseTimeOfDay(String? timeStr) {
    if (timeStr == null || timeStr.toLowerCase() == 'empty' || timeStr.trim().isEmpty) {
    ...
    try {
      ...
      int minute = int.parse(parts[1].split(' ')[0].replaceAll(RegExp(r'[^0-9]'), ''));
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      if (mounted) {
        if (success) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(_isEditing ? 'Doctor updated successfully' : 'Doctor added successfully')),
          );
          context.go('/doctors');
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            ...
            children: [
              ...
              DropdownButtonFormField<int>(
                ...
                items: const [15, 20, 30, 45, 60]
                    .map((min) => DropdownMenuItem(value: min, child: Text('$min minutes')))
                    .toList(),
                onChanged: (value) => setState(() => _selectedSlotDuration = value!),
